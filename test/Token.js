  ...
  describe('Delegated Token Transfers', () => {
    ...
    describe('Success', () => {
      ...
      it('transfers token balances', async () => {
        ...
        balanceOfDeployer.toString().should.be.equal(ethers.utils.parseUnits('999900', 'ether'));

        const balanceOfReceiver = await token.balanceOf(receiver.address);
        balanceOfReceiver.toString().should.be.equal(amount);
      });

      it('resets the allowance', async () => {
        const remainingAllowance = await token.allowance(deployer.address, exchange.address);
        remainingAllowance.toString().should.be.equal('0');
      });
    ...
    describe('Failure', () => {
      it('rejects insufficient amounts', async () => {
        const invalidAmount = tokens(100000000);
        await token.connect(exchange).transferFrom(deployer.address, receiver.address, invalidAmount).should.be
          .reverted;
      });
    });
  });
